...
-- +/- X
CREATE OR REPLACE FUNCTION private.trainer__math_sequence__complexity_0_0(_length int) RETURNS jsonb AS $$
DECLARE
...
-- +/- X  +/- Y
CREATE OR REPLACE FUNCTION private.trainer__math_sequence__complexity_0_1(_length int) RETURNS jsonb AS $$
DECLARE
  X int := floor(random() * 20 - 10);
...
CREATE OR REPLACE FUNCTION private.trainer__math_sequence__complexity_0(_length int) RETURNS jsonb AS $$
BEGIN
  CASE (random()*1)::int
    WHEN 0 THEN
      RETURN private.trainer__math_sequence__complexity_0_0(_length);
    WHEN 1 THEN
      RETURN private.trainer__math_sequence__complexity_0_1(_length);
  END CASE;
END $$ LANGUAGE plpgsql VOLATILE;
