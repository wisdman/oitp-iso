...
CREATE OR REPLACE FUNCTION trainer.math_waste_complexity_2_0(_length int) RETURNS jsonb AS $$
DECLARE
...
CREATE OR REPLACE FUNCTION trainer.math_waste_complexity_2_1(_length int) RETURNS jsonb AS $$
DECLARE
...
CREATE OR REPLACE FUNCTION trainer.math_waste_complexity_2_3(_length int) RETURNS jsonb AS $$
BEGIN
  RETURN (
    WITH d AS (
      SELECT
...
CREATE OR REPLACE FUNCTION trainer.math_waste_complexity_2(_length int) RETURNS jsonb AS $$
BEGIN
  CASE (random()*3)::int
    WHEN 0 THEN
      RETURN trainer.math_waste_complexity_2_0(_length);
    WHEN 1 THEN
      RETURN trainer.math_waste_complexity_2_1(_length);
    WHEN 2 THEN
      RETURN trainer.math_waste_complexity_2_2(_length);
    WHEN 3 THEN
      RETURN trainer.math_waste_complexity_2_3(_length);
  END CASE;
END $$ LANGUAGE plpgsql VOLATILE;
