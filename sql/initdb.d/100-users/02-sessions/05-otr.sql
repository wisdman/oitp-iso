...
CREATE UNLOGGED TABLE private.users_otr (
  ...
  CONSTRAINT private__users_otr__pkey PRIMARY KEY ("owner"),

  CONSTRAINT private__users_otr__check__id CHECK (public.check_unique_id("id")),
  CONSTRAINT private__users_otr__check__options CHECK (jsonb_typeof("options") = 'object'),

  CONSTRAINT private__users_otr__fkey__owner FOREIGN KEY ("owner")
    REFERENCES private.users("id")
    ON UPDATE CASCADE ON DELETE CASCADE
);

CREATE INDEX private__users_otr__idx__ts_expires ON private.users_otr USING btree ("ts", "expires");
