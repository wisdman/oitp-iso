...
-- A = +/- X; B = +/- Y
CREATE OR REPLACE FUNCTION private.trainer__math_sequence__complexity_1_0(_length int) RETURNS jsonb AS $$
DECLARE
...
-- A = +/- X; B = random
CREATE OR REPLACE FUNCTION private.trainer__math_sequence__complexity_1_1(_length int) RETURNS jsonb AS $$
DECLARE
  X int := floor(random() * 20 - 10);
  A int := (CASE WHEN X < 0 THEN floor(random() * 20 + 80) ELSE floor(random() * 9 + 1) END);
BEGIN
...
CREATE OR REPLACE FUNCTION private.trainer__math_sequence__complexity_1(_length int) RETURNS jsonb AS $$
BEGIN
  CASE (random()*1)::int
    WHEN 0 THEN
      RETURN private.trainer__math_sequence__complexity_1_0(_length);
    WHEN 1 THEN
      RETURN private.trainer__math_sequence__complexity_1_1(_length);
  END CASE;
END $$ LANGUAGE plpgsql VOLATILE;
