...
CREATE OR REPLACE FUNCTION trainer.math_middle_complexity_4_0(_quantity int) RETURNS jsonb AS $$
DECLARE
...
-- ABC ( A+B-C X+Y-Z ) XYZ
CREATE OR REPLACE FUNCTION trainer.math_middle_complexity_4_1(_quantity int) RETURNS jsonb AS $$
DECLARE
...
-- ABC ( X+Y-Z A+B-C ) XYZ
CREATE OR REPLACE FUNCTION trainer.math_middle_complexity_4_2(_quantity int) RETURNS jsonb AS $$
DECLARE
  A int; B int; C int; ABC int; X int; Y int; Z int; XYZ int; L int; R int; SUM int;
  RES jsonb := '[]'::jsonb;
BEGIN
...
CREATE OR REPLACE FUNCTION trainer.math_middle_complexity_4(_quantity int) RETURNS jsonb AS $$
BEGIN
  CASE (random()*2)::int
    WHEN 0 THEN
      RETURN trainer.math_middle_complexity_4_0(_quantity);
    WHEN 1 THEN
      RETURN trainer.math_middle_complexity_4_1(_quantity);
    WHEN 2 THEN
      RETURN trainer.math_middle_complexity_4_2(_quantity);
  END CASE;
END $$ LANGUAGE plpgsql VOLATILE;
