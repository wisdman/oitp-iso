...
BEGIN
  ...
  RETURN QUERY (
    ...
    FROM (
      ...
      FROM generate_series(1, _maxItems - random()::int) AS s WHERE _complexity >= 1
      UNION ALL
      SELECT trainer.math_middle_complexity_2(_linesCount) AS "items"
      FROM generate_series(1, _maxItems - random()::int) AS s WHERE _complexity >= 2
      UNION ALL
      SELECT trainer.math_middle_complexity_3(_linesCount) AS "items"
      FROM generate_series(1, _maxItems - random()::int) AS s WHERE _complexity >= 3
      UNION ALL
      SELECT trainer.math_middle_complexity_4(_linesCount) AS "items"
      FROM generate_series(1, _maxItems - random()::int) AS s WHERE _complexity >= 4
      UNION ALL
      SELECT trainer.math_middle_complexity_5(_linesCount) AS "items"
      FROM generate_series(1, _maxItems - random()::int) AS s WHERE _complexity >= 5
      UNION ALL
      SELECT trainer.math_middle_complexity_6(_linesCount) AS "items"
      FROM generate_series(1, _maxItems - random()::int) AS s WHERE _complexity >= 6
    ) AS t
    ORDER BY random()
  );
