CREATE OR REPLACE FUNCTION trainer.words_lexis_antonyms_config() RETURNS SETOF jsonb AS $$
DECLARE
  _trainer public.trainer_type := 'words-lexis-antonyms';
  ...
  RETURN QUERY (
    ...
    FROM (
      ...
      FROM (
        ...
        FROM ONLY trainer.words_lexis_antonyms_data
        WHERE enabled
        ORDER BY random()
