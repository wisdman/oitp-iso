...
CREATE TABLE trainer.text_letters_data (
  ...
  CONSTRAINT text_letters_data__pkey PRIMARY KEY ("id"),
  CONSTRAINT text_letters_data__check__id CHECK ("id" >= 0),

  CONSTRAINT text_letters_data__check__expression CHECK (char_length("expression") > 0),
  CONSTRAINT text_letters_data__check__length CHECK ("length" > 0),
  CONSTRAINT text_letters_data__check__runes CHECK (array_length("runes", 1) > 0)
);

CREATE TABLE trash.text_letters_data() INHERITS (trainer.text_letters_data, private.trash);
...
CREATE INDEX text_letters_data__idx__enabled_public_trainer
  ON trainer.text_letters_data USING btree ("enabled", "public", "trainer");

CREATE INDEX text_letters_data__idx__length
