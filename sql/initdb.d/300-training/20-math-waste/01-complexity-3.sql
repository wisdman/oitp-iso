CREATE OR REPLACE FUNCTION trainer.math_waste_complexity_3_0(_length int) RETURNS jsonb AS $$
BEGIN
  RETURN (
    SELECT
      jsonb_build_object(
...
CREATE OR REPLACE FUNCTION trainer.math_waste_complexity_3_1(_length int) RETURNS jsonb AS $$
DECLARE
...
CREATE OR REPLACE FUNCTION trainer.math_waste_complexity_3_3(_length int) RETURNS jsonb AS $$
BEGIN
  RETURN (
    SELECT
      jsonb_build_object(
...
CREATE OR REPLACE FUNCTION trainer.math_waste_complexity_3(_length int) RETURNS jsonb AS $$
BEGIN
  CASE (random()*3)::int
    WHEN 0 THEN
      RETURN trainer.math_waste_complexity_3_0(_length);
    WHEN 1 THEN
      RETURN trainer.math_waste_complexity_3_1(_length);
    WHEN 2 THEN
      RETURN trainer.math_waste_complexity_3_2(_length);
    WHEN 3 THEN
      RETURN trainer.math_waste_complexity_3_3(_length);
  END CASE;
END $$ LANGUAGE plpgsql VOLATILE;
