CREATE OR REPLACE FUNCTION trainer.words_pairs_config() RETURNS SETOF jsonb AS $$
DECLARE
  _trainer public.trainer_type := 'words-pairs';
  _trainerUI public.trainer_ui := 'words-pairs';
  ...
  RETURN QUERY (
    ...
    FROM (
      ...
      FROM (
        ...
        FROM ONLY trainer.words_pairs_data
        WHERE enabled
        ORDER BY random()
