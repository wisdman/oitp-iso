...
CREATE OR REPLACE FUNCTION private.trainer__math_waste__complexity_1_0(_length int) RETURNS jsonb AS $$
DECLARE
...
CREATE OR REPLACE FUNCTION private.trainer__math_waste__complexity_1_1(_length int) RETURNS jsonb AS $$
DECLARE
...
CREATE OR REPLACE FUNCTION private.trainer__math_waste__complexity_1_2(_length int) RETURNS jsonb AS $$
BEGIN
  RETURN (
    SELECT
      jsonb_build_object(
...
CREATE OR REPLACE FUNCTION private.trainer__math_waste__complexity_1(_length int) RETURNS jsonb AS $$
BEGIN
  CASE (random()*2)::int
    WHEN 0 THEN
      RETURN private.trainer__math_waste__complexity_1_0(_length);
    WHEN 1 THEN
      RETURN private.trainer__math_waste__complexity_1_1(_length);
    WHEN 2 THEN
      RETURN private.trainer__math_waste__complexity_1_2(_length);
  END CASE;
END $$ LANGUAGE plpgsql VOLATILE;
