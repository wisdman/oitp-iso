...
CREATE OR REPLACE FUNCTION self.new_training(_type public.training_type) RETURNS jsonb AS $$
DECLARE
  _timeLimit smallint;
  _trainers jsonb;
  ...
  _trainingId uuid;
BEGIN
  SELECT "timeLimit", "trainers"
  INTO _timeLimit, _trainers
  FROM private.training_configs AS t
  ...
  INSERT INTO private.training("owner", "type", "timeLimit")
  VALUES (current_setting('app.sessionUser')::uuid, _type, _timeLimit)
  RETURNING "id" INTO _trainingId;
    ...
    FROM (
      ...
      FROM (
        SELECT
          ...
          (SELECT array_agg(c) FROM private.trainer_config("trainer"::public.trainer_type) AS t(c jsonb)) AS "configs"
        FROM trainers
        ...
        FROM private.trainer_relax_config((SELECT count(*) FROM trainers)::int / 4) AS c(config jsonb)
      ) AS t
      ORDER BY "idx"
    ) AS t;

  RETURN (
    SELECT to_jsonb(training)
    FROM (
      SELECT
        c."id",
        c."type",
...
CREATE OR REPLACE FUNCTION self.training_finish(_training uuid, _results jsonb) RETURNS smallint AS $$
BEGIN
  ...
  WHERE "training" = _training
    AND "index" = (t."value"->'idx')::smallint;
