...
export class FormRestoreComponent implements OnInit, OnDestroy {
  constructor(
  ...
  form = new FormGroup({
    email: new FormControl("",
                 ...
                 this._loginService.EmailExistsValidator("Email не найден"),
               ),
  }, { })
  ...
  ngOnInit() {
    ...
    this._submitSubscription = this._submitSubject.pipe(
      ...
      switchMap(status => status === "VALID" ? this._loginService.resetPassword(this.form.value) : of(false)),
    ).subscribe(value => {
